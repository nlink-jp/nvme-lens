...
メニューバーにはドライブのシンボルと、ピン留めしたドライブの温度が出ます。正常な間は
メニューバー自身の色で描かれ、**注意が要るときだけ**オレンジや赤になります。常時着色された
アイコンは、目がそれを読み飛ばすよう学習してしまうためです。

クリックするとパネルが開きます。判定の 1 行、ピン留めしたドライブごとの 6 時間グラフ、
その他のドライブは 1 行ずつ。**History…** で長期表示（最大 90 日、温度・寿命・総書込量・
電源断・Unsafe Shutdowns・メディアエラー）に移ります。**収集が止まっていた区間は
線でつながず欠測として描きます。**
...
nvme-lens sample [--format json|table]      1 回計測して記録
...
アプリの設定ウィンドウで行います。メニューバーに出すドライブ、ログイン時に起動、通知、
サンプリング周期、アラート閾値、保持期間。**設定ファイルはありません** — いずれも
「アプリが何をするか」を決める値であり、共有ファイルに置くと設定ウィンドウが表示できても
変更できない値になるだけでした。

CLI には設定がありません。1 回叩けば 1 回記録し、記録済みのものを読み出します。

アプリは単一インスタンスで動作します。2 つ目の起動（例: 通知クリックが
別の場所の .app に解決された場合）は stderr にログを出して即終了し、
実行中のインスタンスには影響しません。CLI サブコマンド（`list` /
`status` / `sample` / `history`）はガードの対象外で、メニューバーアプリの
稼働中もそのまま使えます。
